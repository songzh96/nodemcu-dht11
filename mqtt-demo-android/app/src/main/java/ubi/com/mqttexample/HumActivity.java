...
public class HumActivity extends AppCompatActivity implements Button.OnClickListener{
        ...
        public boolean onNavigationItemSelected(@NonNull MenuItem item) {
            switch (item.getItemId()) {
                case R.id.navigation_home:
                    Intent intent7to6=new Intent(HumActivity.this,TempActivity.class);
                    intent7to6.putExtra("HOST",host);
                    startActivity(intent7to6);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        timer = new Timer();
        // 设置每一秒转动一下
        timer.schedule(new HumActivity.NeedleTask(), 50, 1000);
    ...
    public void onClick(View v) {
      /*  if(v==subButton){
        ...
        if(v==nowButton){
            Intent intent3to4=new Intent(HumActivity.this,ControlActivity.class);
            intent3to4.putExtra("Hum",humidity);
            startActivity(intent3to4);
            subMsg.setText("");

        }
        else if(v==pastButton){
            Intent intent3to5=new Intent(HumActivity.this,HistoryActivity.class);
            intent3to5.putExtra("Hum",humidity);
    ...
    private Handler handler=new Handler(){
        ...
        public void handleMessage(Message msg){
            ...
            if(msg.what==CONNECTED){
                //Toast.makeText(Main7Activity.this,"nice",Toast.LENGTH_SHORT).show();
            }else if(msg.what==LOST){
                //Toast.makeText(Main7Activity.this,"连接丢失，进行重连",Toast.LENGTH_SHORT).show();
            }else if(msg.what==FAIL){
                //Toast.makeText(Main7Activity.this,"连接失败",Toast.LENGTH_SHORT).show();
            }else if(msg.what==RECEIVE){
                message=(String) msg.obj;
                humidity=message.substring(2,4);
                subMsg.setText(humidity);
                //subMsg.append(tempurature);
                b=Float.parseFloat(humidity);
                degree=b*9/5;


            }  if (degree >= 180) {
                //timer.cancel();                degree=180;
                degree=180;
